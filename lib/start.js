            ...
            arrTasks.push(function(callback){
                ...
                if(mobile){
                    switch(cmd){
                        ...
                        case 'press':
                            if(data.path){
                                pushTestCode('press', data.text, data.path, 'await driver.wait(\''+escapeStr(data.path)+'\', 30000).sendElementActions(\'press\');');
                                recorderMobileApp.wait(data.path, 10000).sendElementActions('press').then(doNext).catch(catchError);
                            }
                            else{
                                pushTestCode('press', '', data.x+', '+data.y, 'await driver.sendActions(\'press\', {x: '+data.x+', y: '+data.y+'});');
                                recorderMobileApp.sendActions('press', {x: data.x, y: data.y}).then(doNext).catch(catchError);
                            }
                            break;
                        case 'drag':
                            pushTestCode('drag', '', data.fromX+', '+data.fromY+', '+data.toX+', '+data.toY, 'await driver.sendActions(\'drag\', {fromX: '+data.fromX+', fromY:'+data.fromY+', toX:'+data.toX+', toY:'+data.toY+'});');
                    ...
                    switch(cmd){
                        ...
                        case 'waitBody':
                            ...
                            break;
                        case 'eval':
                            pushTestCode('eval', '', data.replace(/\n/g, ' \\n '), 'await driver.eval(\''+escapeStr(data)+'\');');
