    ...
    if(mobile){
       questions = [
            ...
            },
            {
                ...
                'validate': function(input){
                    return /\.(apk|app|zip)$/.test(input) && (/^https?:\/\//.test(input) || fs.existsSync(input));
                },
                'when': function(answers){
            ...
            arrTasks.push(function(callback){
                ...
                if(mobile){
                    switch(cmd){
                        case 'click':
                            if(data.path){
                                pushTestCode('tap', data.text, data.path, 'return driver.wait(\''+escapeStr(data.path)+'\', 30000).sendElementActions(\'tap\');');
                                recorderMobileApp.wait(data.path, 10000).sendElementActions('tap').then(doNext).catch(catchError);
                            }
                            else{
                                pushTestCode('tap', '', data.x+', '+data.y, 'return driver.sendActions(\'tap\', {x: '+data.x+', y: '+data.y+'});');
                                recorderMobileApp.sendActions('tap', {x: data.x, y: data.y}).then(doNext).catch(catchError);
                            }
                            break;
                        case 'dblClick':
                            if(data.path){
                                pushTestCode('doubleTap', data.text, data.path, 'return driver.wait(\''+escapeStr(data.path)+'\', 30000).sendElementActions(\'doubleTap\');');
                                recorderMobileApp.wait(data.path, 10000).sendElementActions('doubleTap').then(doNext).catch(catchError);
                            }
                            else{
                                pushTestCode('doubleTap', '', data.x+', '+data.y, 'return driver.sendActions(\'doubleTap\', {x: '+data.x+', y: '+data.y+'});');
                                recorderMobileApp.sendActions('doubleTap', {x: data.x, y: data.y}).then(doNext).catch(catchError);
                            }
                            break;
                        case 'press':
                            if(data.path){
                                pushTestCode('press', data.text, data.path, 'return driver.wait(\''+escapeStr(data.path)+'\', 30000).sendElementActions(\'press\');');
                                recorderMobileApp.wait(data.path, 10000).sendElementActions('press').then(doNext).catch(catchError);
                            }
                            else{
                                pushTestCode('press', '', data.x+', '+data.y, 'return driver.sendActions(\'press\', {x: '+data.x+', y: '+data.y+'});');
                                recorderMobileApp.sendActions('press', {x: data.x, y: data.y}).then(doNext).catch(catchError);
                            }
                            break;
                        case 'drag':
                            pushTestCode('drag', '', data.fromX+', '+data.fromY+', '+data.toX+', '+data.toY, 'return driver.sendActions(\'drag\', {fromX: '+data.fromX+', fromY:'+data.fromY+', toX:'+data.toX+', toY:'+data.toY+'});');
                            recorderMobileApp.sendActions('drag', {fromX: data.fromX, fromY: data.fromY, toX: data.toX, toY: data.toY}).then(doNext).catch(catchError);
                            break;
                        case 'sendKeys':
                            pushTestCode('sendKeys', '', data, 'return driver.sendKeys(\''+escapeStr(data)+'\');');
                            recorderMobileApp.sendKeys(data).then(doNext).catch(catchError);
                            break;
                        case 'sleep':
                        ...
                        case 'dismissAlert':
                            ...
                            break;
                        case 'back':
                            pushTestCode('back', '', '', 'return driver.back();');
                        ...
                        case 'expect':
                            co(function*(){
                                ...
                                arrCodes = [];
                                arrCodes.push('return driver.sleep('+expectSleep+').wait(\''+escapeStr(expectPath)+'\', 30000)');
                                switch(expectType){
                                    case 'text':
                                        arrCodes.push('       .text()');
                                        break;
...
function newMobileApp(options, callback){
    ...
    if(deviceList.length === 0){
        capabilities.deviceName = mobilePlatform === 'Android'?'Android Emulator':'iPhone 6';
    }
    else{
