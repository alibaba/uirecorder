    ...
    inquirer.prompt(questions).then(function(anwsers){
        ...
        function onReady(){
            ...
            newChromeBrowser(hosts, true, function*(browser){
                ...
                yield browser.url('chrome-extension://dogndkcimpklgbloegagahcgpcbdepfp/start.html');//  njkfhfdkecbpjlnfmminhmdcakopmcnc
...
function startChromeDriver(){
    ...
    cp.spawn(path.resolve(__dirname,'../tool/'+fileName), ['--url-base=wd/hub', '--port=9766']);
}
...
function newChromeBrowser(hosts, isRecorder, callback){
    ...
    if(isRecorder){
        ...
        capabilities.chromeOptions = {
            args:['disable-bundled-ppapi-flash', 'load-extension=E:\\gitlab\\uirecorder\\chrome-extension']
