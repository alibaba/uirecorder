        ...
        if(defaultOption){
            ...
            questions = questions.concat([
                ...
                },
                {
                    ...
                    'validate': function(input){
                        return input === '1024x768' || /^\d+\s*[x, ]\s*\d+$/.test(input);
                    },
                    'when': function(answers){
                        if(answers.continueRecord){
                            var fileName = cmdFilename || answers.fileName;
                ...
                newChromeBrowser({wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: true, debug: debug}, function*(browser){
                    if(!mobile){
                        if (browserSize) {
                            ...
                            try {
                                yield browser.windowSize(1024, 768);
                            } catch (e) {
                                console.log(e);
                            }
                    ...
                    yield browser.url('chrome-extension://'+(debug?'dloihppejiolohmfmhdceaikmmfmmemf':'donpflclacamajgbnhgjgkkapnlfahlj')+'/'+(mobile?'mobile':'start')+'.html?port='+serverPort);
