...
var defaultBrowserSize = '2560x1600';
    ...
    inquirer.prompt(questions).then(function (answers) {
        ...
        var cmdQueue = async.priorityQueue(function (cmdInfo, next) {
            ...
            arrTasks.push(function (callback) {
                ...
                console.log('lastWindowId:',lastWindowId)
            ...
            arrTasks.push(function (callback) {
                ...
                if (mobile) {
                    switch (cmd) {
                        case 'click':
                            if (data.path) {
                                ...
                                recorderMobileApp.sendActions('tap', {
                                    x: data.x,
                                    y: data.y
                        ...
                        case 'dblClick':
                            if (data.path) {
                                ...
                                recorderMobileApp.sendActions('doubleTap', {
                                    x: data.x,
                                    y: data.y
                                }).then(doNext).catch(catchError);
                            }
                        ...
                        case 'press':
                            if (data.path) {
                                ...
                                recorderMobileApp.sendActions('press', {
                                    x: data.x,
                                    y: data.y,
                        ...
                        case 'drag':
                            ...
                            recorderMobileApp.sendActions('drag', {
                                fromX: data.fromX,
                                fromY: data.fromY,
                                toX: data.toX,
                                toY: data.toY,
                    ...
                    switch (cmd) {
                        case 'contextClick':
                            checkerBrowser && Promise.resolve(doNext).catch(catchError) || doNext();
                            break;
                        case 'dragend':
                            checkerBrowser && Promise.resolve(doNext).catch(catchError) || doNext();
                            break;
        ...
        function onReady(serverPort) {
            ...
            setTimeout(function () {
                newChromeBrowser({
                    ...
                    yield browser.url('chrome-extension://' + (debug ? 'ccdjaknjhllbphkooenbacbbjnejopdo' : 'njkfhfdkecbpjlnfmminhmdcakopmcnc') + '/' + (mobile ? 'mobile' : 'start') + '.html?port=' + serverPort);
            ...
            if (mobile) {
                ...
                function updateMobileInfo() {
                    //console.log('cmdInfo point 6 = ', cmdInfo);
        ...
        var lastCmdInfoTmp = null;
        var contextClickCmdInfo = null;
        ...
        function onCommand(cmdInfo) {
            ...
            function sendKeysFilter(cmdInfo) {
                ...
                if (!mobile && cmd === 'sendKeys') {
                    ...
                    //console.log('2:clickFilter(cmdInfo)');
                    clickFilter(cmdInfo);
                }
                //console.log('4444');
                lastCmdInfo0 = cmdInfo;
                //console.log('lastCmdInfo0:', {lastCmdInfo0})
                //console.log('lastCmdInfo1:', {lastCmdInfo1})
                //console.log('lastCmdInfo2:', {lastCmdInfo2})
            }
            ...
            function clickFilter(cmdInfo) {
                ...
                if (lastCmdInfo1 && lastCmdInfo1.cmd === 'mouseDown') {
                    ...
                    if (cmd === 'mouseUp' &&
                        ...
                        //console.log('clickFilter cmd == mouseUp');
                        // 条件满足，合并为click
                        cmdInfo = {
                        ...
                        //console.log('3:dblClickFilter(lastCmdInfo1);')
            ...
            function dblClickFilter(cmdInfo) {
                ...
                if (lastCmdInfo2 && lastCmdInfo2.cmd === 'click') {
                    ...
                    if (cmd === 'click' &&
                        ...
                        //console.log('dblClickFilter: cmd === \'click\'')
                        // 条件满足，合并为dblClick
                        cmdInfo = {
                            window: cmdInfo.window,
                            frame: cmdInfo.frame,
                        ...
                        // 不需要合并，恢复之前旧的click
                        //console.log('cmdInfo point 3 = ', lastCmdInfo2);
                ...
                if (cmdInfo.cmd !== 'click') {
                    ...
                    //console.log('[分支2]test type of cmdInfo = ',typeof (cmdInfo));
                    //console.log({cmdInfo})
                    //console.log('cmdInfo point 4 = ', cmdInfo);
                    cmdQueue.push(cmdInfo, 2);
                } else {
                    //console.log('分支3')
                    // 500毫秒以内才进行dblClick合并
                    //console.log('[分支3]test type of cmdInfo = ',typeof (lastCmdInfo2));
                    dblClickFilterTimer = setTimeout(function () {
                        //console.log('cmdInfo point 5 = ', lastCmdInfo2);
                        //console.log('cmdQueue point 5 start length', cmdQueue.length);
                        cmdQueue.push(lastCmdInfo2, 2);
                        //console.log('cmdQueue point 5 end length', cmdQueue.length);
                        //console.log('cmdInfo point 5 end');
                        lastCmdInfo2 = null;
                    }, 500);
                }
            ...
            //console.log({cmdInfo})
            if (/^!/.test(cmdInfo.cmd)) {
                //console.log('cmdInfo point 1 = ', cmdInfo);

                cmdQueue.push(cmdInfo, 2);
            } else if((cmdInfo.cmd === 'mouseUp' || cmdInfo.cmd === 'mouseDown') && cmdInfo.data.button === 2){
                if(cmdInfo.cmd === 'mouseDown'){
                    contextClickCmdInfo = cmdInfo;
                }else if(cmdInfo.cmd === 'mouseUp' && contextClickCmdInfo !== null ){
                    contextClickCmdInfo.cmd = 'contextClick';
                    cmdQueue.push(contextClickCmdInfo, 2);
                    contextClickCmdInfo = null;
                }

            } else if (cmdInfo.cmd == 'dragstart'){
                //console.log('dragstart');
                lastCmdInfoTmp = cmdInfo;
                var cmdInfoTmp =  deepCopy(lastCmdInfo0);
                if(lastCmdInfo0 && lastCmdInfo0.cmd === 'mouseDown'){
                    //console.log('mouseUp hahah');
                    cmdInfoTmp.cmd = 'mouseUp';
                    //console.log('cmdInfoTmp:',{cmdInfoTmp})
                    //console.log('lastCmdInfo0:',{lastCmdInfo0})
                    sendKeysFilter(cmdInfoTmp);
                }
            } else if (cmdInfo.cmd == 'dragend' && lastCmdInfoTmp.cmd == 'dragstart') {
                //console.log('dragend');
                cmdInfo.data.x = cmdInfo.data.x - lastCmdInfoTmp.data.x;
                cmdInfo.data.y = cmdInfo.data.y - lastCmdInfoTmp.data.y;
                //console.log('cmdInfo point 2 = ', cmdInfo);
                cmdQueue.push(cmdInfo, 2);
                lastCmdInfoTmp = null;
            } else {
                //console.log('1:call sendKeysFilter');
                sendKeysFilter(cmdInfo);
                //console.log('arrRawCmds: ' + JSON.stringify(arrRawCmds));
            }
        ...
        function saveTestFile() {
            if (allCaseCount > 0) {
                ...
                if (continueRecord) {
                    ...
                    if(arrRawCmdsTarget[1].indexOf('openUrl') !== -1){
                        javaContent = javaContent.replace(/\{\$(openUrl)\}/g, arrRawCmdsTarget[1]+'\r\n' + '                '); 
                        arrRawCmdsTarget[1] = '';
                    }
                    console.log('javaContent1: ' + javaContent);
                    javaContent = javaContent.replace(/\{\$(\w+)\}/g, arrRawCmdsTarget.join('\r\n' + '                '));
                    javaContent = javaContent.replace(/\r\n                \r\n/g, '\r\n');
                    console.log('javaContent2: ' + javaContent);
