        ...
        var cmdQueue = async.priorityQueue(function(cmdInfo, next) {
            ...
            arrTasks.push(function(callback){
                ...
                if(mobile){
                    ...
                    switch(cmd){
                        case 'url':
                            pushTestCode('url', '', data.url, 'return driver.url(_(\''+eacapeStr(data.url)+'\'));');
                            checkerBrowser && checkerBrowser.url(getVarStr(data.url)).then(doNext).catch(catchError) || doNext();
                            break;
                        case 'closeWindow':
