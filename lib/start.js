                ...
                if(mobile){
                    ...
                    switch(cmd){
                        ...
                        case 'waitBody':
                            ...
                            break;
                        case 'eval':
                            pushTestCode('eval', '', data.replace(/\n/g, ' \\n '), 'await driver.eval(_(\''+escapeStr(data)+'\')_;');
                        ...
                        case 'expect':
                            co(function*(){
                                ...
                                arrCodes = [];
                                if(expectType === 'count'){
                                    arrCodes.push('await driver.sleep('+sleepTime+').wait(\''+escapeStr(expectParams[0])+'\', 1000, function(error, elements){');
                                    arrCodes.push('              return error?"0":elements.length.toString();');
                                    arrCodes.push('       })');
                                }
                                else{
                                ...
                                if(checkerBrowser){
                                    var element, value;
                                    if(expectType === 'count'){
                                        value = yield checkerBrowser.sleep(sleepTime).wait(expectParams[0], 1000, function(error, elements){
                                            return error?'0':elements.length.toString();
                                        });
                                    }
                                    else{
                                        if(reDomRequire.test(expectType)){
