...
function startRecorder(locale, cmdFilename){
    if(locale){
    ...
    inquirer.prompt(questions).then(function(anwsers){
        ...
        var cmdQueue = async.queue(function(cmdInfo, next) {
            ...
            arrTasks.push(function(callback){
                ...
                switch(cmd){
                    ...
                    case 'uploadFile':
                        arrCodes = [];
                        arrCodes.push('var element = yield browser.sleep(300).wait(\''+eacapeStr(data.path)+'\', {timeout: 30000});');
        ...
        function onReady(){
            ...
            if(openChecker){
                newChromeBrowser(hosts, false, function*(browser){
                    if(browserSize){
                        yield browser.size(browserSize[0], browserSize[1]);
                    }
                    else{
        ...
        function saveTestFile(){
            ...
            var arrBrowserInitCode = [];
            if(browserSize){
                arrBrowserInitCode.push('yield browser.size('+browserSize[0]+', '+browserSize[1]+');');
            }
            else{
                arrBrowserInitCode.push('yield browser.maximize();');
            }
            arrBrowserInitCode = arrBrowserInitCode.map(function(line){
                return '                '+ line;
            });
            templateContent = templateContent.replace(/\{\$(\w+)\}/g, function(all, name){
                switch(name){
                    ...
                    case 'browserInitCode':
                        return arrBrowserInitCode.join('\r\n');
                }
                return all;
            });
