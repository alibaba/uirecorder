...
function initConfig(options){
    ...
    var configFile = path.resolve('config'+(runtime?'-'+runtime:'')+'.json');
    var config = {};
    ...
    inquirer.prompt(questions).then(function(anwsers){
        ...
        var screenshotsDir = path.resolve('screenshots');
        if(fs.existsSync(screenshotsDir) === false){
            fs.mkdirSync(screenshotsDir);
            console.log('screenshots '.bold+__('dir_created').green);
        }
        var commonsDir = path.resolve('commons');
        if(fs.existsSync(commonsDir) === false){
            fs.mkdirSync(commonsDir);
            console.log('commons '.bold+__('dir_created').green);
        }
    }).catch(function(err){
        console.log(err)
    });
